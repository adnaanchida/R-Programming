...
data <- read_csv("audjudg_r.csv")

# Renaming the columns for clarity
colnames(data) <- c("Method", "Errors")

# One-way ANOVA
anova_result <- aov(Errors ~ Method, data = data)
...
alpha <- 0.05
if (p_value < alpha) {
  cat("The null hypothesis is rejected. The basis for judgment affects the quality of the judgment.")
} else {
  cat("The null hypothesis is accepted. It means that the basis for judgment does not significantly affect the quality of the judgment.")
}
