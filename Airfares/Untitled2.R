...
data <- read_csv("airfares2_r.csv")

# Renaming the columns for clarity
colnames(data) <- c("Flight", "Service", "Cost")

# One-way ANOVA
anova_result <- aov(Cost ~ Service, data = data)
...
alpha <- 0.05
if (p_value < alpha) {
  cat("The null hypothesis is rejected. There are significant differences in the mean price of one-way airline tickets for the three travel agency websites.")
} else {
  cat("The null hypothesis is accepted. It means that there are no significant differences in the mean price of one-way airline tickets for the three travel agency websites.")
}
